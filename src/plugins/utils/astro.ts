...
export const JulianMillennium = JulianCentury * 10;   // Days in Julian millennium
...
export const fixangle = (a: number) => a - 360.0 * (Math.floor(a / 360.0));

/*  FIXANGR  --  Range reduce angle in radians.  */

export const fixangr = (a: number) => a - (2 * Math.PI) * (Math.floor(a / (2 * Math.PI)));

//  DSIN  --  Sine of an angle in degrees

export const dsin = (d: number) => Math.sin(dtr(d));

//  DCOS  --  Cosine of an angle in degrees

export const dcos = (d: number) => Math.cos(dtr(d));
...
export const obliqeq = (jd: number) => {
...
export const nutation = (jd: number) => {
  ...
  */
  ...
  for (let i = 0; i < 5; i++) ta[i] = fixangr(ta[i]);

  const to10 = t / 10.0;
  for (let i = 0; i < 63; i++) {
    let ang = 0;

    for (let j = 0; j < 5; j++) if (nutArgMult[(i * 5) + j] !== 0) ang += nutArgMult[(i * 5) + j] * ta[j];
...
export const deltat = (year: number) => {
  let dt: number;
  ...
  if ((year >= 1620) && (year <= 2000)) {
    ...
    if (year < 948) dt = 2177 + (497 * t) + (44.1 * t * t);
    else {
...
export const equinox = (year: number, which: number) => {
  let JDE0tab;
  let Y;
...
export const equationOfTime = (jd: number) => {
